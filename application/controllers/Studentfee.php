...
class Studentfee extends Admin_Controller
{
    ...
    public function collection_report()
    {
        ...
        $this->form_validation->set_rules('search_type', $this->lang->line('search')." ".$this->lang->line('type'),
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data['results'] = $this->studentfeemaster_model->getFeeCollectionReport($start_date, $end_date,
                    $feetype_id, $received_by, $group);
    ...
    public function search()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $params = array(
                    'class_id'    => $class_id, 'section_id' => $section_id, 'search_type' => $search_type,
    ...
    public function feesearch()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $class_id = $this->input->post('class_id');
            $section_id = $this->input->post('section_id');
            $student_due_fee = $this->studentfee_model->getDueStudentFees($feegroup_id, $fee_groups_feetype_id,
                    $class_id, $section_id);
    ...
    public function reportbyname()
    {
        ...
        if ($this->input->server('REQUEST_METHOD') == "GET") {
        ...
        } else {
            {
                ...
                $student_due_fee = $this->studentfeemaster_model->getStudentFeesByClassSectionStudent($class_id,
        ...
        if ($this->input->server('REQUEST_METHOD') == "GET") {
            ...
            if (!empty($student_result)) {
                foreach ($student_result as $key => $student) {
                    ...
                    $student_due_fee = $this->studentfee_model->getDueFeeBystudentSection($class_id, $section_id,
                            $student_session_id);
    ...
    public function getcollectfee()
    {
        ...
        foreach ($record_array as $key => $value) {
            ...
            $feeList = $this->studentfeemaster_model->getDueFeeByFeeSessionGroupFeetype($fee_session_group_id,
                    $fee_master_id, $fee_groups_feetype_id);
            $fees_array[] = $feeList;
        }
    ...
    public function addfee($id)
    {
        ...
        // Get this students overpayments
        $this->load->model('StudentOverpayment_model');
        $studentoverpaid = $this->StudentOverpayment_model->get($student['id']);
        if ($studentoverpaid === null) {
            $data['amount_overpaid'] = 0;
        } else {
            $data['amount_overpaid'] = $studentoverpaid['amount_overpaid'];

        }
        //End Get this students overpayments
    ...
    public function create()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->session->set_flashdata('msg',
                    '<div studentfee="alert alert-success text-center">'.$this->lang->line('success_message').'</div>');
            redirect('studentfee/index');
        }
    }
    ...
    public function edit($id)
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->studentfee_model->add($data);
            $this->session->set_flashdata('msg',
                    '<div studentfee="alert alert-success text-center">'.$this->lang->line('update_message').'</div>');
            redirect('studentfee/index');
        }
    }
    ...
    public function addstudentfee()
    {
        ...
        $this->form_validation->set_rules('student_fees_master_id', $this->lang->line('fee_master'),
                'required|trim|xss_clean');
        $this->form_validation->set_rules('fee_groups_feetype_id', $this->lang->line('student'),
                'required|trim|xss_clean');
        $this->form_validation->set_rules('amount', $this->lang->line('amount'),
                'required|trim|xss_clean|numeric|callback_check_deposit');
        $this->form_validation->set_rules('amount_discount', $this->lang->line('discount'),
                'required|numeric|trim|xss_clean');
        ...
        if ($this->form_validation->run() == false) {
            ...
            $overpayment = (int) $this->input->post('amount') - (int) $remain_amount;
            if ($overpayment < 0) {
                $overpaymentToadd = 0;
            }else {
                $overpaymentToadd = $overpayment;
            }
            if ($this->StudentOverpayment_model->get($this->input->post('student_id')) !== null) {
                $data = [
                        'id'      => $this->input->post('student_session_id'),
                        'student_id'      => $this->input->post('student_id'),
                        'session_id'      => 1,
                        'amount_overpaid' => $overpaymentToadd
                ];
            }else {
                $data = [
                        'student_id'      => $this->input->post('student_id'),
            ...
            $this->StudentOverpayment_model->add($data);
            ...
            if ($action == "print") {
                ...
                $data['sch_setting'] = $this->sch_setting_detail;
                $fee_record = $this->studentfeemaster_model->getFeeByInvoice($receipt_data->invoice_id,
                        $receipt_data->sub_invoice_id);
    ...
    public function getStuFeetypeBalance($fee_groups_feetype_id, $student_fees_master_id)
    {
        ...
        $array = array(
                'status'      => 'success', 'error' => '', 'balance' => $amount_balance,
    ...
    public function printFeesByGroup()
    {
        ...
        $setting_result = $this->setting_model->get();
        $data['settinglist'] = $setting_result;
        $data['feeList'] = $this->studentfeemaster_model->getDueFeeByFeeSessionGroupFeetype($fee_session_group_id,
                $fee_master_id, $fee_groups_feetype_id);
        $data['sch_setting'] = $this->sch_setting_detail;
    ...
    public function printFeesByGroupArray()
    {
        ...
        $data['feearray'] = $fees_array;
        $this->load->model('StudentOverpayment_model');
        $data['overpayment'] = $this->StudentOverpayment_model->get($studentId)['amount_overpaid'] ?? 0;
    ...
    {
        $this->form_validation->set_rules('fee_session_groups', $this->lang->line('fee_group'),
                'required|trim|xss_clean');
    ...
    {
        $this->form_validation->set_rules('fee_groups_feetype_id', $this->lang->line('fee_groups_feetype_id'),
                'required|trim|xss_clean');
        $this->form_validation->set_rules('student_fees_master_id', 'student_fees_master_id',
                'required|trim|xss_clean');
        ...
        if ($this->form_validation->run() == false) {
            ...
            $array = array(
                    'status'               => 'success', 'error' => '', 'balance' => $remain_amount,
                    'discount_not_applied' => $discount_not_applied, 'remain_amount_fine' => $remain_amount_fine
            );
            echo json_encode($array);
        }
    }
        ...
        if (is_numeric($this->input->post('amount')) && is_numeric($this->input->post('amount_discount'))) {
            if ($this->input->post('amount') != "" && $this->input->post('amount_discount') != "") {
                if ($this->input->post('amount') < 0) {
                    $this->form_validation->set_message('check_deposit',
                            $this->lang->line('deposit_amount_can_not_be_less_than_zero'));
                    return false;
                }
                return true;
            }
            ...
            $this->form_validation->set_message('check_deposit',
                    $this->lang->line('amount')." ".$this->lang->line('field_must_contain_only_numbers'));
        ...
        if ($this->form_validation->run() == false) {
            ...
            foreach ($total_row as $total_row_key => $total_row_value) {
                ...
                $json_array = array(
                        ...
                        'date'            => date('Y-m-d',
                                $this->customlib->datetostrtotime($this->input->post('collected_date'))),
        ...
        if ($this->input->server('REQUEST_METHOD') == "POST") {
            ...
            if (!empty($students_list)) {
                foreach ($students_list as $student_key => $student_value) {
                    $students_list[$student_key]['fees_list'] = $this->studentfeemaster_model->studentDepositByFeeGroupFeeTypeArray($student_key,
                            $student_value['fee_groups_feetype_ids']);
